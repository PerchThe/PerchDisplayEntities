...
import perch.displayentities.APlugin;
import perch.displayentities.Util;
import perch.displayentities.YMLConfig;
...
public abstract class SubCmd {
    ...
    public ComponentBuilder getHelp(ComponentBuilder base, CommandSender sender, String alias) {
        String help = ChatColor.GOLD + "/" + alias + " " + ChatColor.GOLD + this.name + " ";
        base.append(help + getLanguageString("params", "", sender).replace(ChatColor.RESET.toString(),
                        ChatColor.GOLD.toString())).event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, ChatColor.stripColor(help)))
