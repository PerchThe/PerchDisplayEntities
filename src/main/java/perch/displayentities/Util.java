...
public class Util {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                entities.forEach((en -> {
                    if (en.isValid())
                        if (i % 2 != 0)
                            player.showEntity(PerchDisplayEntities.get(), en);
                        else
                            player.hideEntity(PerchDisplayEntities.get(), en);
                }));
                i++;
            }
    ...
    public static void logToFile(String message) {
        try {
            File dataFolder = PerchDisplayEntities.get().getDataFolder();
            if (!dataFolder.exists())
                dataFolder.mkdir();
            Date date = new Date();
            File saveTo = new File(PerchDisplayEntities.get().getDataFolder(),
                    "logs" + File.separatorChar
                            + new SimpleDateFormat(
                            PerchDisplayEntities.get().getConfig().loadMessage("log.file-format", "yyyy.MM.dd", false))
                            .format(date)
                            + ".log");
            ...
            pw.println(new SimpleDateFormat(
                    PerchDisplayEntities.get().getConfig().loadMessage("log.log-date-format", "[dd.MM.yyyy HH:mm:ss]", false))
                    .format(date)
                    + message);
            pw.flush();
            pw.close();
    ...
    public static boolean hasBannedWords(Player user, String text) {
        if (user.hasPermission("DisplayEntities.bypass.censure"))
            return false;
        String message = ChatColor.stripColor(ChatColor.translateAlternateColorCodes('&', text.toLowerCase(Locale.ENGLISH)));
        for (String regex : PerchDisplayEntities.get().getConfig().getStringList("blocked.regex"))
            if (Pattern.compile(regex).matcher(message).find()) {
                if (PerchDisplayEntities.get().getConfig().getBoolean("blocked.log.console", true))
                ...
                if (PerchDisplayEntities.get().getConfig().getBoolean("blocked.log.file", true))
                    logToFile("user: '" + user.getName() + "' attempt to write '" + text
                            + "' (stripped by colors and lowcased to '" + message + "') was blocked by regex: '" + regex
                            + "'");
                sendMessage(user,
                        PerchDisplayEntities.get().getLanguageConfig(user).loadMessage("blocked-by-censure", "", null, true));
                return true;
            }
        for (String bannedWord : PerchDisplayEntities.get().getConfig().getStringList("blocked.words"))
